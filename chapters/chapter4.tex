...
\begin{itemize}
    ...
    \item \textbf{\texttt{scenarios/}} — Layer 2: six domain modules, each a self-contained Python file (\texttt{scenario\_e1.py}, \texttt{scenario\_e2.py}, \texttt{scenario\_m1.py}, \texttt{scenario\_t1.py}, \texttt{scenario\_e2m1.py}, \texttt{scenario\_m1t1.py}).
...
The physical models (solar irradiance, wind power curve, BESS state-of-charge update) are defined in Section~\ref{sec:scenario-e1} (Equations~\ref{eq:solar_power}--\ref{eq:soc_charge}). Battery dispatch is handled by \texttt{calculate\_storage\_dispatch()}: when net power is negative (renewable surplus), batteries charge; when positive, they discharge proportionally across all units.

\subsection{Scenario E2 --- Electric Vehicle Charging Infrastructure}

\texttt{EVChargingFederate} models 100 electric vehicles on a modified IEEE 13-node feeder. The charging strategy is injected as a \texttt{ChargingStrategy} enum value at construction time, enabling the same \texttt{EVChargingFederate} class to run \texttt{UNCOORDINATED}, \texttt{SMART}, and \texttt{V2G} algorithms without code duplication. The arrival distributions and strategy algorithms are defined in Section~\ref{sec:scenario-e2} (Equations~\ref{eq:tou}--\ref{eq:urgency}).
...
The adaptive signal controller implements the green-time allocation formula defined in Section~\ref{sec:scenario-m1} (Equation~\ref{eq:adaptive_signal}). In fixed-timing mode the same \texttt{TrafficSignal} class is used with equal phase weights ($Q_{NS} = Q_{EW} = 1$ regardless of actual queues), requiring no additional class or branching in the main loop. The \texttt{adaptive\_signals} boolean injected at construction time selects between the two behaviours. Emissions are tracked per-vehicle as defined in Equation~\ref{eq:emissions}.

\subsection{Scenario T1 --- 5G Slice Resource Allocation}

\texttt{NetworkSlicingSimulation} models three gNBs arranged in a triangular layout serving 200 mobile users distributed across three 5G network slices: 100~eMBB users, 40~URLLC users, and 60~mMTC users. User mobility follows the random-waypoint model at pedestrian speed (0.5--2.0~m/s). Serving-cell assignment and handover use the 3GPP Urban Macro path-loss model defined in Section~\ref{sec:scenario-t1} (Equation~\ref{eq:path_loss_t1}), with a 3~dB hysteresis margin and a 10-step cooldown to prevent ping-pong effects. The \texttt{SliceResourceAllocator} implements the \texttt{STATIC} and \texttt{DYNAMIC} slicing strategies defined in Section~\ref{sec:scenario-t1} (Equations~\ref{eq:static_alloc}--\ref{eq:qos_t1}), selected via the \texttt{SlicingStrategy} enum at construction time.
...
The central agent is \texttt{EVTrafficVehicle}, which extends the M1 \texttt{Vehicle} class with an E2 \texttt{ElectricVehicle} battery model. It progresses through the four-phase lifecycle defined in Section~\ref{sec:scenario-e2m1} (Equation~\ref{eq:ev_phases}). In coupled mode, 500~EV vehicles share the grid with 2{,}000 background vehicles, so EV travel times are affected by traffic congestion. In uncoupled mode, EVs teleport instantly to their assigned charging station, providing a baseline that isolates the grid charging behaviour from mobility effects.
...
\subsection{Scenario M1+T1 --- Cross-Domain Mobility--Telecommunications Integration}

\texttt{CrossDomainM1T1} inherits from \texttt{BaseFederate} and reuses components from both \texttt{scenario\_m1.py} and \texttt{scenario\_t1.py} through direct Python imports, following the same intra-process federation pattern established by \texttt{CrossDomainE2M1}. The scenario runs at 1-second resolution for 3~hours, matching M1's native time step; T1's resource-allocation and handover logic execute once per M1 tick rather than at the standalone 100\,ms cadence.

The central agent is \texttt{ConnectedVehicle}, which extends the M1 \texttt{Vehicle} class and holds a T1 \texttt{MobileUser} instance. At every time step, \texttt{ConnectedVehicle.sync\_position()} converts the vehicle's current grid node to T1 metres (Equation~\ref{eq:m1t1_mapping}) and writes the result to the embedded \texttt{MobileUser}, replacing RWP mobility for that user. The T1 path-loss model, handover logic, and RB allocation then operate on the vehicle-derived positions without any modification to the \texttt{scenario\_t1.py} classes.

Listing~\ref{lst:connectedvehicle} illustrates the dual-domain agent.
...
class ConnectedVehicle(M1Vehicle):
    ...
    def sync_position(self):
        """Copy M1 grid coordinates into T1 continuous space."""
        self.user.x = self.current_position[0] * self._spacing_m
        self.user.y = self.current_position[1] * self._spacing_m
\end{minted}
\caption{\texttt{ConnectedVehicle} dual-domain agent (scenario\_m1t1.py)}
\label{lst:connectedvehicle}
\end{listing}

The QoS feedback path is implemented in \texttt{CrossDomainM1T1.\_apply\_qos\_feedback()}, which computes the per-gNB URLLC satisfaction rate after each T1 step and marks any gNB below the 80\% threshold as degraded. The \texttt{TrafficSignal.update()} calls for intersections within degraded cells receive equal-demand weights ($Q_{NS} = Q_{EW} = 1$) rather than measured queue lengths, reverting those signals to fixed-time behaviour for the duration of the degradation event.

In coupled mode, 2{,}300 background vehicles and 200 connected vehicles share the M1 traffic grid; the 200 connected vehicles also serve as T1 users driven by real vehicle positions. In uncoupled mode, the 200 T1 users move under independent RWP and the QoS feedback path is disabled, providing a baseline that treats mobility and telecommunications as independent domains.


\section{User Interface}

A key design goal of the framework is that researchers and practitioners should be able to run, configure, and compare simulation scenarios without writing code. To this end, the third architectural layer is a web-based graphical dashboard built with Streamlit~\cite{streamlit}, a Python library that converts ordinary scripts into interactive applications served at \texttt{localhost:8501}. The dashboard exposes all six implemented scenarios through a unified interface: users select a scenario from a dropdown, adjust its parameters interactively, choose a strategy or coupling mode, and launch the simulation with a single button click. Results are rendered immediately in the browser as structured metric grids and bar charts, and a collapsible log panel allows inspection of intermediate simulation progress. No knowledge of the underlying Python classes, HELICS configuration, or command-line tooling is required. The dashboard is launched with:
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figs/UIM1_light.png}
    \caption{Streamlit user interface showing scenario selection, parameter configuration, and simulation results.}
    \label{fig:ui_streamlit_overview}
\end{figure}

The Streamlit application in \texttt{app.py} provides a single-page dashboard that covers all six scenarios. A sidebar selector switches between scenarios and exposes scenario-specific parameters through \texttt{st.number\_input} and \texttt{st.selectbox} widgets. Strategy and coupling-mode selectors map user choices to the corresponding enum values before calling the scenario entry-point function. A ``Compare all strategies'' checkbox triggers the \texttt{compare\_*} family of functions, which execute all strategy variants sequentially and return a dictionary of reports keyed by strategy name.
...
Scenario~M1+T1 faced a different coordinate-space mismatch: M1's $5 \times 5$ grid spans 5{,}000\,m per axis while T1's area was originally 2{,}000\,m per side. The resolution was to rescale T1's \texttt{area\_size} to 5{,}000\,m and adjust gNB positions proportionally (Equation~\ref{eq:m1t1_mapping}), preserving the triangular inter-site geometry while eliminating the scale mismatch without modifying either source module.
...
Table~\ref{tab:m1t1_results} contrasts the coupled and uncoupled modes of the Mobility--Telecommunications cross-domain scenario.

\begin{table}[h]
    \centering
    \caption{Scenario M1+T1 coupling mode comparison}
    \label{tab:m1t1_results}
    \begin{tabular}{lrrl}
        \toprule
        \textbf{Metric} & \textbf{Uncoupled} & \textbf{Coupled} & \textbf{Change} \\
        \midrule
        \multicolumn{4}{l}{\textit{Telecommunications metrics}} \\
        ...
        Handover rate (events/min)        & 749.1 & 867.6  & $+15.8\%$ \\
        Handover success rate (\%)        & 95.0  & 95.0   & ---        \\
        Per-gNB load std dev (users)      & 15.6  & 7.0    & $-55.1\%$ \\
        \midrule
        \multicolumn{4}{l}{\textit{Mobility metrics (all vehicles)}} \\
        Avg.\ travel time (s)             & 541.0 & 559.3  & $+3.4\%$   \\
        Avg.\ delay (s)                   & 22.7  & 41.0   & $+80.9\%$  \\
        Signal degradation events         & 0     & 25{,}915 & ---        \\
        Total degradation duration (s)    & 0     & 124{,}664 & ---       \\
        Intersections affected            & 0     & 14     & ---          \\
        Total CO$_2$ emissions (kg)       & 2{,}827 & 2{,}933 & $+3.7\%$ \\
        \bottomrule
    \end{tabular}
\end{table}

The M1+T1 results reveal three distinct cross-domain effects. First, vehicular mobility produces substantially more handovers than pedestrian-speed random-waypoint users: coupled mode records 156{,}168 handover attempts versus 134{,}843 in uncoupled mode---a 15.8\% increase. The high absolute rate (749--868 events/min) reflects the T1 model's independent per-step shadow fading with a 4\,dB standard deviation that frequently exceeds the 3\,dB hysteresis threshold; the coupled premium arises because vehicles traverse a 1{,}250\,m link in approximately 90\,s at 50\,km/h, crossing cell boundaries far more rapidly than pedestrians. The per-gNB load standard deviation falls from 15.6~users (uncoupled) to 7.0~users (coupled)---a 55.1\% decrease---because the traffic network's structured grid distributes vehicles more uniformly across the three gNB coverage areas than unconstrained random-waypoint users, which cluster in ways that happen to be spatially uneven.

QoS satisfaction collapses in coupled mode: eMBB falls from 39.0\% to 15.4\%, URLLC from 39.4\% to 19.2\%, and mMTC from 40.5\% to 16.9\%---reductions of 51--61\%. The primary driver is the burst-loading pattern produced when red-light releases simultaneously dispatch a wave of vehicles into a gNB's coverage area. At those instants the per-gNB active user count spikes far above the time-averaged value, and the dynamic allocator's proportional rule cannot assign each user the two RBs required for QoS; rounding losses are also amplified, reducing overall RB utilisation from 73.8\% to 60.1\%. By contrast, the uncoupled pedestrian model distributes users continuously and smoothly, producing more consistent per-gNB loads.

The T1$\rightarrow$M1 feedback path is the most operationally significant effect. Because coupled URLLC QoS (19.2\%) remains well below the 80\% degradation threshold throughout almost the entire simulation, the feedback mechanism is continuously active: 25{,}915 per-gNB threshold violations across the 10{,}800-step run affect 14 of 25 intersections for a combined 124{,}664 intersection-seconds of fixed-time operation. With 14 intersections reverting to fixed-time signals, the average background-vehicle delay rises from 22.7\,s (uncoupled) to 41.0\,s (coupled)---an 80.9\% increase. The additional idling raises total fleet CO$_2$ emissions by 3.7\% (2{,}827\,kg to 2{,}933\,kg). This constitutes a causal chain only observable through coupled simulation: vehicular RF clustering at intersection nodes degrades URLLC QoS, which suppresses adaptive signal control across 14 intersections, which increases congestion and emissions. No domain-specific model of either traffic or telecommunications in isolation could expose this interaction.
