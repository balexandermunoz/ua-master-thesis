...
Modern cities rely on the efficient operation and coordination of critical infrastructure systems across three primary domains: energy distribution, mobility networks, and telecommunications. In the Ubiwhere Urban Management Platform context, these domains are treated as urban verticals and are increasingly interdependent—electric vehicles link energy and mobility systems, smart traffic management requires robust telecommunications, and renewable energy integration depends on communication networks for grid coordination.

Urban Management Platforms have emerged as essential tools for city administrators and infrastructure operators, providing integrated perspectives of territorial dynamics and enabling data-driven decision-making. These platforms aggregate real-time data from multiple verticals, analyze historical trends, and support strategic planning through predictive analytics and artificial intelligence. Ubiwhere's Urban Management Platform, launched in 2018, exemplifies this integrated approach by providing comprehensive land management capabilities aligned with international standards and best practices~\cite{ubiwhere_urban_platform}. The platform enables stakeholders to move beyond siloed domain management toward holistic urban governance that recognizes cross-domain dependencies.
...
\subsection{Digital Twins for Urban Infrastructure Management}

Digital twins have emerged as a key paradigm for moving urban platforms from descriptive monitoring toward predictive and prescriptive decision support~\cite{digital_twin_cities}. In this context, a digital twin is not only a visual replica of city assets, but a continuously synchronized computational representation that combines real-time data, historical context, and simulation models to estimate current system state and anticipate future behavior. This shift is essential for city management tasks that require proactive orchestration across interconnected systems, such as load balancing under EV growth, adaptive mobility control during incidents, and coordinated responses to communication failures.

State-of-the-art digital twin approaches emphasize a closed loop between sensing, modeling, simulation, and operational action. First, heterogeneous data streams update the twin in near real time; second, what-if and forecasting simulations evaluate candidate interventions; finally, selected control actions are deployed and their outcomes are fed back into the model for continuous calibration. Under this view, the value of digital twins lies in enabling timely, system-level decision making under uncertainty, rather than providing dashboards alone. However, achieving this capability at city scale remains challenging because most simulation tools are still domain-specific, making multi-domain orchestration difficult without co-simulation mechanisms.
...
In this dissertation, the term \textit{standard} refers to broadly adopted, formalized specifications for model exchange and co-simulation interoperability. Under this criterion, FMI/FMU is the primary standard directly aligned with the heterogeneous simulation workflow considered in this work. Other relevant solutions used in practice for urban infrastructure studies (e.g., Mosaik, MECSYCO, and HELICS) are implementation frameworks rather than formal interoperability standards, and are therefore discussed in the following subsection.
